...
export default function AudioEngine() {
  ...
  useEffect(() => {
    ...
    const isSilenceRecoveryEnabled = adv?.silenceRecovery?.isEnabled;
    const intervalMs = adv?.silenceRecovery?.sensitivity === 'high' ? 2000 : 
                       adv?.silenceRecovery?.sensitivity === 'medium' ? 8000 : 20000;
    
    // Fallback to safe interval if not enabled
    const finalInterval = isSilenceRecoveryEnabled ? intervalMs : 15000;
      ...
      if (isPlaying && !isRenderingChunk) {
        ...
        if (mainAudio && mainAudio.src && !mainAudio.ended) {
          if (mainAudio.paused) {
             ...
             // Dynamic threshold based on recovery settings
             const threshold = isSilenceRecoveryEnabled ? 1 : 2;

             if (stallCount.current['main'] > threshold) {
               console.warn('[Safety] Detected unexplained pause in playback, nudging...');
               if (isSilenceRecoveryEnabled) {
                 // Use custom retry delay if enabled
                 setTimeout(() => {
                    mainAudio.play().catch(() => {});
                 }, adv.silenceRecovery.retryDelayMs);
               } else {
                 mainAudio.play().catch(() => {});
               }
             }
  ...
  }, [isPlaying, isForeground, currentTrack, getTrackUrl, isRenderingChunk, settings.advancedAudio]);
  ...
  useEffect(() => {
    ...
    if (subAudioRef.current) {
      ...
      const volume = s.volume * layerGain * masterGainMultiplier * safetyMultiplier * masterAudioMultiplier * safeListeningCap;
    ...
    if (mainAudioRef.current) {
      ...
      const masterAudioMultiplier = settings.globalModes.masterAudioEnabled ? 1.0 : 0.0;
      const safeListeningCap = settings.advancedAudio.safeListening.isEnabled ? settings.advancedAudio.safeListening.maxVolumeCap : 1.0;
      const volume = settings.mainVolume * layerGain * safetyMultiplier * masterAudioMultiplier * safeListeningCap;
      mainAudioRef.current.volume = Math.min(1.0, Math.max(0.0, volume));
    }
  }, [settings.audioTools.gainDb, settings.mainVolume, settings.globalModes, settings.pureHz, settings.binaural, settings.isochronic, settings.solfeggio, settings.schumann, settings.didgeridoo, settings.shamanic, settings.noise, settings.nature, settings.mentalToughness, safetyMultiplier]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('binaural', isLayerPlaying, 'binaural', { 
        ...
        pitchSafeMode: settings.binaural.pitchSafeMode || settings.globalModes.pitchSafeMode
      }, settings.binaural.volume, settings.binaural.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.binaural.isEnabled, settings.binaural.playInBackground, settings.binaural.leftFreq, settings.binaural.rightFreq, settings.binaural.volume, settings.binaural.gainDb, settings.fadeInOut, settings.binaural.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      syncBgLayer('noise', isLayerPlaying, 'noise', { 
        ...
        pitchSafeMode: settings.noise.pitchSafeMode || settings.globalModes.pitchSafeMode
      }, settings.noise.volume, settings.noise.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.noise.isEnabled, settings.noise.playInBackground, settings.noise.type, settings.noise.frequency, settings.noise.pitchSafeMode, settings.noise.volume, settings.noise.gainDb, settings.fadeInOut]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('didgeridoo', isLayerPlaying, 'didgeridoo', { 
        ...
        pitchSafeMode: settings.didgeridoo.pitchSafeMode || settings.globalModes.pitchSafeMode
      }, settings.didgeridoo.volume, settings.didgeridoo.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.didgeridoo.isEnabled, settings.didgeridoo.playInBackground, settings.didgeridoo.frequency, settings.didgeridoo.depth, settings.didgeridoo.volume, settings.didgeridoo.gainDb, settings.fadeInOut, settings.didgeridoo.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('shamanic', isLayerPlaying, 'shamanic', { 
        ...
        pitchSafeMode: settings.shamanic.pitchSafeMode || settings.globalModes.pitchSafeMode
      }, settings.shamanic.volume, settings.shamanic.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.shamanic.isEnabled, settings.shamanic.playInBackground, settings.shamanic.frequency, settings.shamanic.depth, settings.shamanic.playbackRate, settings.shamanic.volume, settings.shamanic.gainDb, settings.fadeInOut, settings.shamanic.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('pureHz', isLayerPlaying, 'pureHz', { 
        frequency: settings.pureHz.frequency, 
        pitchSafeMode: settings.pureHz.pitchSafeMode || settings.globalModes.pitchSafeMode 
      }, settings.pureHz.volume, settings.pureHz.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.pureHz.isEnabled, settings.pureHz.playInBackground, settings.pureHz.frequency, settings.pureHz.volume, settings.pureHz.gainDb, settings.fadeInOut, settings.pureHz.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('isochronic', isLayerPlaying, 'isochronic', { 
        ...
        pitchSafeMode: settings.isochronic.pitchSafeMode || settings.globalModes.pitchSafeMode
      }, settings.isochronic.volume, settings.isochronic.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.isochronic.isEnabled, settings.isochronic.playInBackground, settings.isochronic.frequency, settings.isochronic.pulseRate, settings.isochronic.volume, settings.isochronic.gainDb, settings.fadeInOut, settings.isochronic.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('solfeggio', isLayerPlaying, 'solfeggio', { 
        frequency: settings.solfeggio.frequency, 
        pitchSafeMode: settings.solfeggio.pitchSafeMode || settings.globalModes.pitchSafeMode 
      }, settings.solfeggio.volume, settings.solfeggio.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.solfeggio.isEnabled, settings.solfeggio.playInBackground, settings.solfeggio.frequency, settings.solfeggio.volume, settings.solfeggio.gainDb, settings.fadeInOut, settings.solfeggio.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('schumann', isLayerPlaying, 'pureHz', { 
        frequency: settings.schumann.frequency, 
        pitchSafeMode: settings.schumann.pitchSafeMode || settings.globalModes.pitchSafeMode 
      }, settings.schumann.volume, settings.schumann.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.schumann.isEnabled, settings.schumann.playInBackground, settings.schumann.frequency, settings.schumann.volume, settings.schumann.gainDb, settings.fadeInOut, settings.schumann.pitchSafeMode]);
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      await syncBgLayer('mentalToughness', isLayerPlaying, 'mentalToughness', { 
        ...
        pitchSafeMode: settings.mentalToughness.pitchSafeMode || settings.globalModes.pitchSafeMode
    ...
    const releaseWakeLock = async () => {
      if (wakeLockRef.current) {
        try {
          ...
          wakeLockRef.current = null; // Clear ref anyway to prevent stuck state
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const sync = async () => {
      ...
      await syncBgLayer('nature', isLayerPlaying, 'nature', { 
        ...
        pitchSafeMode: settings.nature.pitchSafeMode || settings.globalModes.pitchSafeMode
      }, settings.nature.volume, settings.nature.gainDb);
    };
    sync();
  }, [isPlaying, settings.globalModes, settings.nature.isEnabled, settings.nature.playInBackground, settings.nature.type, settings.nature.frequency, settings.nature.pitchSafeMode, settings.nature.volume, settings.nature.gainDb, settings.nature.normalize, settings.fadeInOut, safetyMultiplier]);
